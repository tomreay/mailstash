...
import {EmailAccountWithFolders, FolderSyncPayload, JobResult} from '../types';
...
export const folderSyncHandler: Task = async (
  ...
  try {
    ...
    if (!account.isActive) {
      console.log(`[folder-sync] Account ${accountId} is not active, skipping sync`);
      return;
    }
  ...
  do {
    ...
    for (const message of result.messages) {
      ...
      if (emailsProcessed % 10 === 0) {
        console.log(`[folder-sync] Progress: ${emailsProcessed} emails processed`);
      }
    }
