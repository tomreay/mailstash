...
export const mboxImportHandler: Task = async (payload) => {
  ...
  console.log(`[mbox-import] Starting import for account ${accountId} from ${mboxFilePath}`)
  
  // Check if file exists before proceeding
  const fileExists = await fs.access(mboxFilePath).then(() => true).catch(() => false)
  ...
  try {
    ...
    console.log(`[mbox-import] Set sync status to 'syncing' for account ${accountId}`)
    ...
    for await (const emailData of parser.parseMessages(mboxFilePath)) {
      try {
        ...
        if (processed === 1) {
          console.log(`[mbox-import] Successfully processed first email: ${emailMessage.subject || 'No subject'}`)
        }
        ...
        if (processed % batchSize === 0 || (now - lastProgressLog) > 5000) {
          ...
          console.log(`[mbox-import] Progress: ${processed} emails - ${failed} failed`)
          lastProgressLog = now
        }
      } catch (error) {
        failed++
    ...
    console.log(`[mbox-import] Import completed: ${processed} imported, ${failed} failed`)
    
    // Clean up temporary file ONLY on successful import
    try {
      await fs.unlink(mboxFilePath)
      console.log(`[mbox-import] Successfully cleaned up temporary file after import: ${mboxFilePath}`)
    } catch (error) {
      console.error(`[mbox-import] Failed to clean up temporary file: ${error}`)
    ...
    console.log(`[mbox-import] Keeping mbox file after error for debugging: ${mboxFilePath}`)
