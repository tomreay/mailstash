...
export class StatsService {
   ...
   */
  static async getUserStats(userId: string, accountId?: string): Promise<StatsResponse> {
    // Get user's email account(s)
    const accounts = await AccountsDAO.findAccountsWithSyncStatus(userId, accountId)
   ...
   */
  private static async getAggregatedAccountsStats(accounts: Array<{
    id: string
    syncStatus?: {
      lastSyncAt?: Date | null
      syncStatus?: string | null
    ...
    return StatsDAO.formatAggregatedStats(stats, mostRecentSync, overallSyncStatus)
  }
}