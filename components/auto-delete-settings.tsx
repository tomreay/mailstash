...
export function AutoDeleteSettings({
  ...
  const deleteSummary = useMemo(() => {
    ...
    if (deleteDelayHours !== null) {
      ...
      conditions.push(
        `${preset?.label || `${deleteDelayHours} hours`} after import`
      );
    }
    ...
    if (deleteAgeMonths !== null) {
      ...
      conditions.push(
        `older than ${preset?.label || `${deleteAgeMonths} months`}`
      );
    }
    ...
    return (
      <div className={cn('space-y-6', className)}>
        <Alert>
          ...
          <AlertDescription>{deleteSummary}</AlertDescription>
        </Alert>
          ...
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              <p className='text-sm text-amber-700 mt-1'>
                Emails matching your rules are being permanently deleted during
                each sync. To change settings, you must first disable
                auto-delete.
              </p>
            </div>
          </div>
  ...
  return (
    <div className={cn('space-y-6', className)}>
          ...
          {dryRunStatus ? (
            ...
            <div>
              ...
              <p className='text-sm text-blue-700 mt-1'>
                Set up your deletion rules below. You&apos;ll be able to preview
                which emails would be deleted before enabling auto-delete.
              </p>
            </div>
          )}
        </div>
        ...
        <div className='space-y-2'>
          ...
          <div className='grid grid-cols-4 gap-2'>
            {DELAY_PRESETS.slice(0, 4).map(preset => (
              <Button
                key={preset.hours}
                variant={
                  deleteDelayHours === preset.hours ? 'default' : 'outline'
                }
                size='sm'
          ...
          <div className='flex items-center gap-2'>
            <Input
              ...
              onChange={e =>
                onDelayChange(e.target.value ? parseInt(e.target.value) : null)
              }
              className='w-36'
            />
          ...
          <div className='grid grid-cols-4 gap-2'>
            {AGE_PRESETS.slice(0, 4).map(preset => (
              <Button
                key={preset.months}
                variant={
                  deleteAgeMonths === preset.months ? 'default' : 'outline'
                }
                size='sm'
          ...
          <div className='flex items-center space-x-2'>
            <Checkbox
              ...
              onCheckedChange={checked =>
                onArchivedOnlyChange(checked as boolean)
              }
            />
            <Label
              htmlFor='archived-only'
      ...
      {hasValidRules ? (
        <Button onClick={onRunDryRun} disabled={isLoading} className='w-full'>
          {isLoading ? (
            <>
              <Loader2 className='h-4 w-4 mr-2 animate-spin' />
              Starting Dry-Run...
